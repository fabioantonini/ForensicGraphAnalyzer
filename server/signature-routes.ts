...
export function registerSignatureRoutes(router: Router) {
  ...
  // Endpoint per rigenerare completamente l'analisi di tutte le firme
  router.post("/signature-projects/:id/regenerate-analysis", isAuthenticated, async (req, res) => {
    console.error(`\n===== REGENERATE ANALYSIS ENTRY =====`);
    console.error(`TIMESTAMP: ${new Date().toISOString()}`);
    console.error(`PROGETTO ID: ${req.params.id}`);
    ...
    try {
      ...
      console.error(`[REGENERATE] Inizio rigenerazione completa per progetto ${projectId}`);
      
      // Ottieni tutte le firme del progetto
      const signatures = await storage.getProjectSignatures(projectId);
      const referenceSignatures = signatures.filter(s => s.isReference && s.processingStatus === 'completed');
      const verificationSignatures = signatures.filter(s => !s.isReference && s.processingStatus === 'completed');
      
      console.error(`[REGENERATE] Firme di riferimento: ${referenceSignatures.length}`);
      console.error(`[REGENERATE] Firme da verificare: ${verificationSignatures.length}`);
      
      if (referenceSignatures.length === 0) {
        return res.status(400).json({ error: 'Nessuna firma di riferimento disponibile' });
      }
      
      const results = [];
      
      // Rigenera tutti i confronti forzando l'uso del Python analyzer
      for (const signature of verificationSignatures) {
        console.error(`[REGENERATE] Elaborando firma ${signature.id}: ${signature.originalFilename}`);
        
        try {
          // Forza l'analisi Python per rigenerare parametri
          const referenceSignature = referenceSignatures[0];
          const signaturePath = path.join('./uploads', signature.filename);
          const referencePath = path.join('./uploads', referenceSignature.filename);
          
          console.error(`[REGENERATE] Confronto Python tra:`);
          console.error(`  - Verifica: ${signaturePath}`);
          console.error(`  - Riferimento: ${referencePath}`);
          
          const comparisonResult = await SignaturePythonAnalyzer.compareSignatures(
            signaturePath,
            referencePath,
            { widthMm: signature.realWidthMm || 50, heightMm: signature.realHeightMm || 20 },
            { widthMm: referenceSignature.realWidthMm || 50, heightMm: referenceSignature.realHeightMm || 20 },
            true, // Genera sempre report
            {
              caseName: project.name,
              subject: `Rigenerazione ${signature.originalFilename}`,
              date: new Date().toLocaleDateString('it-IT'),
              documentType: 'Verifica di firma',
              notes: 'Rigenerazione automatica parametri'
            },
            projectId
          );
          
          console.error(`[REGENERATE] Risultato Python per firma ${signature.id}:`);
          console.error(`  - Similarità: ${comparisonResult.similarity}`);
          console.error(`  - Grafico: ${comparisonResult.comparison_chart ? 'GENERATO' : 'MANCANTE'}`);
          console.error(`  - Parametri: ${comparisonResult.verifica_parameters ? 'AGGIORNATI' : 'MANCANTI'}`);
          ...
          // Aggiorna il confronto e i dati avanzati
          await storage.updateSignatureComparisonResult(signature.id, comparisonResult.similarity);
          
          if (comparisonResult.comparison_chart || comparisonResult.description) {
            const updateData: Record<string, any> = {};
            if (comparisonResult.comparison_chart) updateData.comparisonChart = comparisonResult.comparison_chart;
            if (comparisonResult.description) updateData.analysisReport = comparisonResult.description;
          ...
          console.error(`[REGENERATE] ✅ Firma ${signature.id} completamente rigenerata`);
          
        } catch (error) {
          console.error(`[REGENERATE] ❌ Errore rigenerazione firma ${signature.id}:`, error);
          results.push(signature); // Restituisci la firma originale
        }
      }
      
      console.error(`[REGENERATE] ✅ Rigenerazione completata. Firme elaborate: ${results.length}`);
      res.json(results);
      
    } catch (error: any) {
      console.error(`[REGENERATE] ❌ Errore generale:`, error);
      res.status(500).json({ error: error.message });
    }
  });
