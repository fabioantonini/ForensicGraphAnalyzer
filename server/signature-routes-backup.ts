...
// Genera report PDF per tutte le firme da verificare in un progetto
  ...
  appRouter.post("/signature-projects/:id/compare-all", async (req, res) => {
...
export function registerSignatureRoutes(appRouter: Router) {
  ...
  appRouter.post("/signature-projects/:id/compare-all", isAuthenticated, isActiveUser, async (req, res) => {
    ...
    // TEST ENDPOINT - Restituisce dati semplici per debug
    console.error(`✅ ENDPOINT COMPARE-ALL FUNZIONANTE - Restituisco dati test`);
    res.json([{
      id: 158,
      message: "ENDPOINT COMPARE-ALL RAGGIUNTO CORRETTAMENTE",
      timestamp: new Date().toISOString(),
      user: req.user?.username
    }]);
  });
  
  console.log("🔥🔥🔥 SIGNATURE ROUTES REGISTERED SUCCESSFULLY 🔥🔥🔥");
}
