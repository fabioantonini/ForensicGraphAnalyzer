...
export async function initializeChromaDB() {
  ...
  try {
    ...
    try {
      ...
      // If we're here, all attempts failed - try to use pgvector if available
      log(`Impossibile avviare o connettersi al server ChromaDB`, "chromadb");
      
      // Verifichiamo che PostgreSQL e pgvector siano disponibili
      ...
      if (pgConnected) {
        ...
        if (vectorAvailable) {
          log(`Database PostgreSQL con estensione pgvector disponibile, sistema fallback attivato`, "chromadb");
          
          // Aggiungi o crea la tabella document_embeddings se non esiste ancora
          try {
            await db.execute(sql`
              CREATE TABLE IF NOT EXISTS document_embeddings (
                ...
                embedding vector(1536), -- dimensione standard per embedding OpenAI Ada-002
            ...
            log(`Struttura tabella document_embeddings verificata`, "chromadb");
          } catch (tableError) {
            log(`Errore nella verifica della tabella document_embeddings: ${tableError}`, "chromadb");
          }
      ...
      // Se pgvector non è disponibile, usiamo il fallback in-memory
      log(`Sistema fallback in-memory attivato (pgvector non disponibile)`, "chromadb");
      isChromaAvailable = false;
      return false;
    }
  } catch (error) {
