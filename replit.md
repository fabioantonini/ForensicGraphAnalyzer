...
- **GMAIL SMTP INTEGRAZIONE COMPLETA**: Implementato sistema Gmail SMTP come alternativa gratuita a SendGrid. Creato `gmail-service.ts` con supporto nodemailer, endpoint amministrazione (`/api/admin/gmail-config`, `/api/admin/test-gmail`), sistema fallback intelligente (Gmail → SendGrid), email templating professionale con styling, configurazione sicura tramite App Password Google. Sistema email unificato con priorità Gmail per eliminare dipendenza da servizi a pagamento.
- **MIGRAZIONE SENDGRID → GMAIL COMPLETATA (Agosto 2025)**: Rimosso completamente SendGrid dal sistema. Sostituito tab "SendGrid" con "Gmail SMTP" nel pannello admin. Pulito server/admin-routes.ts da tutti i riferimenti SendGrid. Sistema ora 100% basato su Gmail SMTP gratuito (500 email/giorno). Aggiunta raccomandazione per Gmail dedicato (es: grapholexinsight@gmail.com) invece di account personali per maggiore professionalità.
- **RESET PASSWORD DEPLOYMENT-READY COMPLETATO (Agosto 2025)**: Sistema reset password Gmail completamente funzionante e testato. Risolto routing universale per supportare sia `/reset-password/{token}` che query string. Implementata logica intelligente URL che rileva automaticamente ambiente Replit vs sviluppo locale. Email ora generate con URL corretti per deployment. Sistema completamente operativo e pronto per produzione senza configurazioni aggiuntive.
- **WAKE UP QUIZ FALLBACK OPENAI RISOLTO (Agosto 2025)**: Risolto problema critico "Failed to start quiz" per brunapascali e tutti gli utenti. Implementato sistema fallback intelligente che usa automaticamente la chiave OpenAI di fabioantonini quando utenti non hanno chiavi personali. Corretto bug parametro `userId` in `generateQuizQuestions`. Sistema ora completamente funzionante per tutti gli utenti con logging dettagliato per troubleshooting. Testato e confermato funzionamento con brunapascali e grapholexinsight.
...
- **RIPRISTINO CONFIGURAZIONE OPENAI QUIZ (Agosto 2025)**: Ripristinati parametri originali Wake Up Quiz dopo test creatività. Temperature tornata a 0.8 standard, rimossi parametri aggiuntivi (top_p, frequency_penalty), prompt semplificato al sistema base. Configurazione stabile e funzionante come da implementazione originale.
- **FALLBACK OPENAI ESTESO A RAG SYSTEM (Agosto 2025)**: Esteso sistema fallback intelligente dalla chiave di fabioantonini anche all'Assistente Documentale (RAG). Risolto errore "Invalid OpenAI API key" che impediva l'uso del sistema di ricerca semantica. Ora tutti i sistemi (Wake Up Quiz, RAG, anonimizzazione) usano automaticamente la chiave globale senza configurazione utente.
